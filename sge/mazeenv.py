import os
import sys
from sge.subtaskgraph import SubtaskGraph
...
class MazeEnv(object):  # single batch
    def __init__(self, game_name, graph_param, game_len, gamma, render_config={}):
        if game_name == 'playground':
            ...
            # graph_folder = C:\Data\Env\ICML\subtask-graph-execution-master\sge\data\subtask_graph_play
            graph_folder = os.path.join('.', 'data', 'subtask_graph_play')
            # filename = C:\Data\Env\ICML\subtask-graph-execution-master\sge\data\subtask_graph_play\train_1
            filename = 'play_{param}'.format(param=graph_param)

        elif game_name == 'mining':
            from sge.mining import Mining
            game_config = Mining()
            # graph_folder = C:\Data\Env\ICML\subtask-graph-execution-master\sge\data\subtask_graph_mining
            graph_folder = os.path.join('.', 'data', 'subtask_graph_mining')
            # filename = C:\Data\Env\ICML\subtask-graph-execution-master\sge\data\subtask_graph_mining\train_1
            filename = 'mining_{param}'.format(param=graph_param) 

        self.game_config = game_config
        self.max_task = self.game_config.nb_subtask_type
        self.subtask_list = self.game_config.subtask_list
        ...
        # graph & map
        self.subtaskgraph = SubtaskGraph(graph_folder, filename, self.max_task)  # just load all graph
        self.mazemap = Mazemap(game_name, game_config, render_config)
        self.gamma = gamma
    ...
    def step(self, action):
        if self.subtaskgraph.graph_index is None:
        ...
        oid = self.mazemap.act(action)
        if (action, oid) in self.game_config.subtask_param_to_id:  # if (action, item) is one of the subtasks
            sid = self.game_config.subtask_param_to_id[(action, oid)]
        ...
        # pritn()
        # print("self._get_state():",self._get_state())
        # print("self._get_info()():",self._get_info())
        # 

        return self.render(), self.reward, (self.game_over or self.time_over), [] #self._get_info()

    def reset(self, seed=None, graph_index=None):  # after every episode
        if seed is not None:
            np.random.seed(seed)
        if graph_index is None:
            graph_index = np.random.permutation(self.subtaskgraph.num_graph)[0]
        else:
            graph_index = graph_index % self.subtaskgraph.num_graph

        # 1. reset graph
        if graph_index >= 0:
            self.subtaskgraph.set_graph_index(graph_index)
            self.nb_subtask = len(self.subtaskgraph.subtask_id_list)
            self.rew_mag = self.subtaskgraph.rew_mag
            self.subtask_id_list = self.subtaskgraph.subtask_id_list

        # 2. reset subtask status
        self.executed_sub_ind = -1
        self.game_over = False
        self.time_over = False
        self.mask, self.mask_id = np.ones(self.nb_subtask, dtype=np.uint8), np.zeros(self.max_task, dtype=np.uint8)
        for ind, sub_id in self.subtaskgraph.ind_to_id.items():
            self.mask_id[sub_id] = 1
        self.completion, self.comp_id = np.zeros(self.nb_subtask, dtype=np.int8), np.zeros(self.max_task, dtype=np.uint8)
        self._compute_elig()
        self.step_count, self.ret, self.reward = 0, 0, 0

        # 3. reset map
        self.mazemap.reset(self.subtask_id_list)
        #if self.rendering:
        #    self.render()
        return self.render() #self._get_state() #, self._get_info()

    def state_spec(self):
        return [
            {'dtype': self.mazemap.get_obs().dtype, 'name': 'observation', 'shape': self.mazemap.get_obs().shape},
    ...
    def get_actions(self):
        return self.game_config.legal_actions
    ...
    def render(self):
        ...
        color = []
        for ind, sub_id in enumerate(self.subtaskgraph.subtask_id_list):
            if self.completion[ind] == 1:  # success
                ...
                color.append('white')
        #self.subtaskgraph.draw_graph(self.game_config, self.rew_mag, color)
        text_lines, text_widths, status, bg_colors = self._get_status()
        return self.mazemap.render(self.step_count, text_lines, text_widths, status, bg_colors)
    ...
    def _get_state(self):
        ...
        return {
            'observation': self.mazemap.get_obs(),
    ...
    def _act_subtask(self, sub_id):
        ...
        if sub_id < 0:
            return reward
        sub_ind = self.subtaskgraph.id_to_ind[sub_id]
    ...
    def _compute_elig(self):
        self.eligibility = self.subtaskgraph.get_elig(self.completion)
        self.elig_id = get_id_from_ind_multihot(
            self.eligibility, self.subtaskgraph.ind_to_id, self.max_task)
    ...
    def _get_status(self, reward=None):
        ...
        for ind, sub_id in enumerate(self.subtaskgraph.subtask_id_list):
            (action, oid) = self.game_config.subtask_param_list[sub_id]
            action_key = self.game_config.operation_list[action]['key']
            if 'name' in self.game_config.subtask_list[sub_id]:
                sub_name = self.game_config.subtask_list[sub_id]['name']
            else:
                sub_name = self.action_meanings[action]+' ' + \
                    self.game_config.object_param_list[oid]['name']
            if self.completion[ind] == 1:  # success
                color = GREEN
            elif self.mask[ind] == 0:  # fail
                color = DARK_RED
